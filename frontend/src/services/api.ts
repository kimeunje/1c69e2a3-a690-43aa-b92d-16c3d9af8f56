...
const mockApprovalRequests: any[] = [
  ...
  {
    id: 3, vulnerability_id: 1, requester_id: 3, approver_id: 2,
    due_date: '2025-03-30', action_plan: '전체 쿼리 파라미터 바인딩 적용', status: 'approved',
    approved_at: '2025-03-18T09:00:00', created_at: '2025-03-15T11:00:00',
    vulnerability: mockVulnerabilities[0], requester: brief(mockUsers[2]), approver: brief(mockUsers[1]),
  },
]
...
function qp(c: AxiosRequestConfig): Record<string, string> {
  const r: Record<string, string> = {}
  if (c.params) Object.entries(c.params).forEach(([k, v]) => { if (v != null) r[k] = String(v) })
  return r
}

function body(c: AxiosRequestConfig): any {
...
const routes: { m: string; p: string; h: H }[] = [
  ...
  { m: 'delete', p: '/users/:id', h: async (_c, p) => {
    ...
    if (i === -1) fail('사용자를 찾을 수 없습니다.', 404)
  ...
  { m: 'get', p: '/vulnerabilities', h: async (c) => {
    const p = qp(c)
    let f = [...mockVulnerabilities]
    if (p.assessment_id) f = f.filter((v) => v.assessment_id === +p.assessment_id)
    if (p.status) f = f.filter((v) => v.status === p.status)
    if (p.assignee_id) f = f.filter((v) => v.assignee_id === +p.assignee_id)
    if (p.category) f = f.filter((v) => v.category === p.category)
    const pg = parseInt(p.page || '0'), sz = parseInt(p.size || '20')
    return ok({ items: f.slice(pg * sz, pg * sz + sz), total: f.length })
  }},
  ...
  { m: 'post', p: '/approvals/:id/approve', h: async (_c, p) => {
    ...
    if (!r) fail('결재 요청을 찾을 수 없습니다.', 404)
    r.status = 'approved'
    const v = mockVulnerabilities.find((x) => x.id === r.vulnerability_id)
  ...
  { m: 'get', p: '/dashboard/stats', h: async () => {
    const t = mockVulnerabilities.length, d = mockVulnerabilities.filter((v) => v.status === 'done').length, ip = mockVulnerabilities.filter((v) => v.status === 'in_progress').length
    return ok({
